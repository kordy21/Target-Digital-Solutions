  ...
  return (
    <section className="relative w-full overflow-hidden flex flex-col items-center justify-center">
      ...
      <div className="w-full bg-background rounded-3xl border border-border shadow-sm p-8 md:p-12 lg:p-16 flex flex-col items-center">
        ...
        {/* Blog Grid */}
        <StaggerContainer className="w-full grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {blogItems.map((item) => {
            const tags: string[] = t.raw(`items.${item.id}.tags`);

            return (
              <StaggerItem key={item.id}>
                <div className="relative group w-full h-112.5 md:h-125 rounded-[24px] overflow-hidden border border-border bg-card text-card-foreground shadow-sm hover:shadow-lg transition-all duration-500 hover:-translate-y-2 cursor-pointer">
                    ...
                    <div className="flex flex-col items-start w-full">
                      ...
                      <div className="flex flex-wrap gap-2">
                        {tags.map((tag: string, idx: number) => (
                          <span 
                            key={idx} 
                            className="px-4 py-1.5 rounded-full text-[12px] md:text-[13px] font-semibold font-cairo bg-secondary text-secondary-foreground group-hover:bg-white/20 group-hover:text-white group-hover:backdrop-blur-md transition-colors border border-transparent group-hover:border-white/30"
                          >
                            {tag}
                          </span>
                        ))}
